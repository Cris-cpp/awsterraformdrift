...
1. Verify AWS MCP is connected by attempting a simple AWS MCP call (e.g., list EC2 instances or describe S3 buckets). If the call fails or no AWS MCP server is available, **stop immediately** and tell the user:
   ...
   > "Connect AWS MCP first — see terraform-drift-analyzer/references/usage.md for setup instructions. Do not proceed until AWS MCP is connected."
...
```bash
python3 terraform-drift-analyzer/scripts/terraform_parser.py <terraform_directory>
```
...
```bash
python3 terraform-drift-analyzer/scripts/aws_discovery.py drift-workspace/T.json normalize drift-workspace/mcp_response.json
```
...
```bash
python3 terraform-drift-analyzer/scripts/report_generator.py drift-workspace/diff.json drift-workspace/architecture.mmd --output report.md
```
